package com.myself.mall.search.service.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.myself.common.to.es.SkuEsModel;
import com.myself.common.utils.R;
import com.myself.mall.search.config.MallSearchConfig;
import com.myself.mall.search.constant.EsConstant;
import com.myself.mall.search.feign.ProductFeignService;
import com.myself.mall.search.service.MallService;
import com.myself.mall.search.vo.AttrResponseVo;
import com.myself.mall.search.vo.BrandVo;
...
public class MallServiceImpl implements MallService {
	...
	public SearchResult search(SearchParam param) {
		...
		try {
			// 2.执行检索请求
			SearchResponse response = restHighLevelClient.search(searchRequest, MallSearchConfig.COMMON_OPTIONS);
	...
	private SearchRequest buildSearchRequest(SearchParam param) {
		...
		// 3.1 聚合出当前所有的attrId
		TermsAggregationBuilder attrIdAgg = AggregationBuilders.terms("attr_id_agg").field("attrs.attrId.keyword");
	...
	private SearchResult buildSearchResult(SearchResponse response, SearchParam param) {
		...
		ParsedStringTerms attr_id = attr_agg.getAggregations().get("attr_id_agg");
		...
		// 4.当前商品所有涉及到的分类信息
		ParsedStringTerms catalog_agg = response.getAggregations().get("catalog_agg");
		...
		int totalPages = (int)Math.ceil(total / EsConstant.PRODUCT_PASIZE);
